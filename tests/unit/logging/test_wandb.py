...
class TestTensorboardLogging:
    ...
    def test_create_tensorboard_logging(self, backend_class, tmpdir):
        logger = TensorboardLogging(tmpdir, backend_class, "tensorboard")
    ...
    def test_log_scalar(self, mocked_sw, tmpdir):
        logger = TensorboardLogging(tmpdir, mocked_sw, "tb")
    ...
    @mock.patch("torch.utils.tensorboard.SummaryWriter")
    def test_log_image(self, mocked_sw, tmpdir):
        logger = TensorboardLogging(tmpdir, mocked_sw, "tb")
        tag = "an_image"
    ...
    @mock.patch("torch.utils.tensorboard.SummaryWriter")
    def test_log_video(self, mocked_sw, tmpdir):
        logger = TensorboardLogging(tmpdir, mocked_sw, "tb")
        tag = "a_video"
    ...
    @mock.patch("torch.utils.tensorboard.SummaryWriter")
    def test_log_histogram(self, mocked_sw, tmpdir):
        logger = TensorboardLogging(tmpdir, mocked_sw, "tb")
        tag = "a_histogram"
        data = np.random.rand(3, 24, 24)
        step = 10
    ...
    @mock.patch("torch.utils.tensorboard.SummaryWriter")
    def test_log_grads(self, mocked_sw, tmpdir):
        logger = TensorboardLogging(tmpdir, mocked_sw, "tb")
    ...
    @mock.patch("torch.utils.tensorboard.SummaryWriter")
    def test_log_video_from_frames(self, mocked_sw, tmpdir):
        logger = TensorboardLogging(tmpdir, mocked_sw, "tb")
        tag = "a_video"
